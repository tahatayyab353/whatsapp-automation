...
interface AppointmentItem {
  ...
  calendar_sync_status?: "synced" | "pending" | "failed" | "disconnected";
...
const SAMPLE_APPOINTMENTS: AppointmentItem[] = [
  ...
  },
  {
    ...
    calendar_last_synced_at: "2026-09-05T14:00:05Z",
    reminders: [
      {
        reminder_type: "APPOINTMENT_24H",
        status: "sent",
  ...
  },
  {
    ...
    calendar_sync_status: "synced",
    calendar_last_synced_at: "2026-09-05T15:00:00Z",
    reminders: [
      {
        reminder_type: "APPOINTMENT_24H",
        status: "sent",
...
export default function AppointmentsPage() {
  ...
  const getCalendarSyncBadge = (status?: string) => {
    switch (status) {
      case "synced":
  ...
  return (
    ...
    <div className="min-h-screen bg-slate-50">
      {/* Top Nav Bar */}
      <header className="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-xs mb-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <div>
            ...
            <p className="text-sm text-slate-500">
              ...
              Manage patient bookings, automated WhatsApp reminders, and Google/Outlook calendar synchronization.
            </p>
          </div>
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <div className="text-right flex flex-col items-end gap-1">
                          ...
                          {appt.calendar_sync_status && (
                            <span
                              className={`inline-block px-1.5 py-0.5 text-[10px] font-medium rounded border uppercase tracking-wider ${getCalendarSyncBadge(
                                appt.calendar_sync_status
                              )}`}
                            >
                              📅 {appt.calendar_sync_status}
                          ...
                          <div suppressHydrationWarning className="text-xs text-slate-500 mt-0.5">
                            {mounted ? (
                              <>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-4">
            ...
            {selectedAppt ? (
              <div className="space-y-4">
                ...
                {/* External Calendar Sync (Chunk 12) */}
                <div className="pt-2 border-t border-slate-100 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">
                      Calendar Sync
                    </span>
                    {selectedAppt.calendar_sync_status && (
                      <span
                        className={`px-1.5 py-0.5 text-[10px] font-semibold rounded-full border uppercase ${getCalendarSyncBadge(
                          selectedAppt.calendar_sync_status
                        )}`}
                      >
                        {selectedAppt.calendar_sync_status}
                  ...
                  <div className="p-2.5 bg-slate-50 rounded-lg border border-slate-100 text-xs flex items-center justify-between">
                    <span className="text-slate-600 text-[11px]">External Calendar Event:</span>
                    <span className="font-medium text-slate-800 text-[11px]">
                      {selectedAppt.calendar_sync_status === "synced"
                        ? "Synchronized"
                        : selectedAppt.calendar_sync_status === "pending"
                        ? "Pending Sync"
                        : "Not Synced"}
                    </span>
                  </div>
